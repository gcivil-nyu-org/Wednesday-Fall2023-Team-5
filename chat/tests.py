...
class TestModelSave(TestCase):
    def setUp(self):
        ...
        self.user1.userprofile.images = UserImages.objects.create(
            user_profile=self.user1.userprofile, image="profileImages/great-gatsby-background-1.jpg"
        )
        self.user1.save()
        self.user2.save()

    def test_save(self):
        Thread.objects.create(first_user=self.user1, second_user=self.user2,
                              updated=datetime.utcnow())
        t = Thread.objects.filter(Q(first_user=self.user1) & Q(second_user=self.user2))
        self.assertEqual(t[0].first_user_image_url, "/media/profileImages/great-gatsby-background-1.jpg")
