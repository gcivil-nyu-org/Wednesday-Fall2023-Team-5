...
def pre_process_data(data):
    ...
    data_encoded = data.join(pd.DataFrame(mlb.fit_transform(data['userprofile__interests']), columns=mlb.classes_))

    # one hot encode remaining categorical columns
    data_encoded = pd.get_dummies(data_encoded, columns=['userprofile__drink_pref', 'userprofile__smoke_pref',
...
def get_recommendations(knn, data, user_attributes, num_recommendations=5):
    ...
    recommended_users = data.iloc[indices[0]].drop(columns=[col for col in data.columns if col != 'userprofile__user'])
...
def get_knn_recommendations(data, curr_user):
    ...
    user_index = data_encoded[data_encoded['userprofile__user'] == curr_user].index.values.astype(int)[0]
    print('user_index:', user_index)
    user_attributes = data_encoded.iloc[user_index]
    # print(user_attributes)
    recommendations, distances = get_recommendations(knn, data_encoded, user_attributes, num_recommendations=3)
